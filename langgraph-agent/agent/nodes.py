...
async def agent_node_streaming(state: AgentState) -> Dict[str, Any]:
    """
    Streaming agent node - processes user input and streams LLM tokens via state
    This node streams tokens into state["streaming_tokens"] for consumption by graph.astream()
    """
    ...
    streaming_tokens = state.get("streaming_tokens", [])
    full_content = ""
